...
def FaceDetectionImage(prototxtpath,modelpath,imagepath,minconfidence):
    ...
    net = cv2.dnn.readNetFromCaffe(prototxt= prototxtpath,caffeModel= modelpath)  #(args["prototxt"], args["model"])

    # load the input image and construct an input blob for the image
    # by resizing to a fixed 300x300 pixels and then normalizing it
    image = cv2.imread(filename= imagepath) #(args["image"])
    (h, w) = image.shape[:2]
